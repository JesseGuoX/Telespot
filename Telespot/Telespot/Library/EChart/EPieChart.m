...
@property (nonatomic, strong) UICountingLabel *contributionLable;
...
//        [_contentView addSubview:title];
           ...
           self.nameLable = [[UILabel alloc] initWithFrame:self.frame];
//        self.nameLable .text = @"Dec";
        self.nameLable.textAlignment = NSTextAlignmentCenter;
        self.nameLable.font = [UIFont fontWithName:@"Menlo-Bold" size:15];
        self.nameLable.textColor = [UIColor whiteColor];
        self.nameLable.center = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds) * 0.8);
        [_contentView addSubview:self.nameLable];

        
        UIView *line = [[UIView alloc] initWithFrame:self.bounds];
        line.backgroundColor = [UIColor whiteColor];
        line.bounds = CGRectMake(0, 0, CGRectGetWidth(self.bounds) * 0.6, 2);
        line.center = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds) * 1);
        [_contentView addSubview:line];
        
        self.contributionLable = [[UICountingLabel alloc] initWithFrame:self.frame];
        self.contributionLable.center = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds) * 1.2);
        self.contributionLable.textAlignment = NSTextAlignmentCenter;
        self.contributionLable.method = UILabelCountingMethodEaseInOut;
        self.contributionLable.font = [UIFont fontWithName:@"Menlo" size:13];
        self.contributionLable.textColor = [UIColor whiteColor];
        self.contributionLable.format = @"%i";
        [_contentView addSubview:self.contributionLable];
//        [self.contributionLable countFrom:0 to:_ePieChartDataModel.budget withDuration:2.0f];
        
//        UICountingLabel *budgetLabel = [[UICountingLabel alloc] initWithFrame:self.frame];
//        budgetLabel.center = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds) * 1.2);
...
#pragma mark  添加 Setter and Getter
-(void)setName:(NSString *)name
{
    _name = name;
    self.nameLable.text = _name;
}

-(void)setContribution:(NSNumber *)contribution
{
    _contribution = contribution;
    [self.contributionLable countFrom:0 to:_contribution.integerValue withDuration:2.0f];
    [self reloadContent];
